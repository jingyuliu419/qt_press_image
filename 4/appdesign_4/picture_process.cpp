...
//    cv::Mat block1 = gaosiImage(src, cv::Size(15, 15));
    cv::Mat resizedImage = biCubicInterp1(src, times);
    cv::Mat block0 = gaosiImage(resizedImage, cv::Size(0, 0));
...
cv::Mat PictureProcess::gaosiImage(cv::Mat src, cv::Size kernelSize) {
    cv::Mat result;
    GaussianBlur(src, result, kernelSize, 15, 0,cv::BORDER_REFLECT);
