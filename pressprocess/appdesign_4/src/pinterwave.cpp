...
void PlotWidget::paintEvent(QPaintEvent *event) {
    ...
    blackFormat.setFontPointSize(12);  // 黑色字体
    blackFormat.setForeground(QBrush(QColor(0, 0, 0)));

    QTextCharFormat redFormat;
    redFormat.setFontPointSize(40);  // 红色字体
    redFormat.setForeground(QBrush(QColor(255, 0, 0)));

    int classCounts[5] = {0, 0, 0, 0, 0};
    ...
    // 生成动态文件名
    QString textName = ui->lineEdit1->text();
    QString fileName = QString("./result_%1.txt").arg(textName);

    QFile resultFile(fileName);
    if (!resultFile.exists()) {
        qDebug() << "File does not exist. Creating: " << fileName;
        if (!resultFile.open(QIODevice::WriteOnly)) {
            qDebug() << "Failed to create the file: " << fileName;
            return;
        }
        resultFile.close();
    }
    ...
    // 写入标题和统计数据
    outFile << "文件名称: " << fileName.toStdString() << "\n";
    outFile << "按压结果统计：\n";
    outFile << "----------------------\n";
    outFile << "class1 (正确): " << classCounts[0] << "\n";
    outFile << "class2 (肘臂姿态错误): " << classCounts[1] << "\n";
    outFile << "class3 (施力位置偏移): " << classCounts[2] << "\n";
    outFile << "class4 (施力分散): " << classCounts[3] << "\n";
    outFile << "class5 (手掌姿态错误): " << classCounts[4] << "\n";
    outFile << "----------------------\n";
    outFile << "数据分析完成。\n";
    outFile.close();

    // qDebug() << "Inference results written to " << fileName;
    cursor.insertText("over!!!!\n", blackFormat);
    ...
    // QString fileName1 = QString("./bestresult_%1.txt").arg(textName);
    // QFile resultFile1(fileName1);
    // if (!resultFile1.exists()) {
    //     qDebug() << "File does not exist. Creating: " << fileName;
    //     if (!resultFile1.open(QIODevice::WriteOnly)) {
    //         qDebug() << "Failed to create the file: " << fileName;
    //         return;
    //     }
    //     resultFile1.close();
    ...
    // // 写入结果
    // std::ofstream outFile1(fileName1.toStdString());
    // if (!outFile.is_open()) {
    //     qDebug() << "Failed to open result file for writing.";
    //     return;
    // }
    // // std::ofstream resultFile1("/home/ljy/Doc1/mycode/5/appdesign_5/result/resultxx.txt");
    ...
    //         outFile1 << "Set " << k << ": Invalid images, skipping inference.\n";
    //         continue;
    //     }
    ...
    //     outFile1 << "类别" << k << ": " << predictedClass.toStdString() << "\n";
    // }
    // outFile1 << "统计：" << "\n";
    // outFile1 << "class1:" <<numClass1<<  "\n";
    // outFile1 << "class2:" <<numClass2<<  "\n";
    // outFile1 << "class3:" <<numClass3<<  "\n";
    // outFile1 << "class4:" <<numClass4<<  "\n";
    // outFile1 << "class5:" <<numClass5<<  "\n";
