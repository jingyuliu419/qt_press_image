...
public:
    ...
    void writeResultsToFile(const QString &textName, const int *classCounts);
    void handleErrorClass(const QString &resultText, QTextCursor &cursor,
                                  const QTextCharFormat &redFormat, int *classCounts);
    void handleProcessedImage(const cv::Mat &image, const std::set<double> &pointSet,
                                      QTextCursor &cursor, int *classCounts,
                                      const QTextCharFormat &redFormat, const QTextCharFormat &blackFormat);
    QTextCharFormat createTextFormat(const QColor &color, int fontSize);
